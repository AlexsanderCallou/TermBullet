...
namespace TermBullet.Tests.Services.History;

public sealed class JsonHistoryMaintenanceServiceTests
{
    [Fact]
    ...
    private static JsonHistoryMaintenanceService CreateService(string root)
    {
        return new JsonHistoryMaintenanceService(
            root,
            new MonthlyJsonPathResolver(root),
            new JsonFileStore());
    }
