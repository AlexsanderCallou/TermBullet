...
namespace TermBullet.Tests.Services.DataTransfer;
...
public sealed class JsonDataTransferServiceTests
{
    ...
    public async Task ExportAsync_writes_monthly_files_and_settings_to_package()
    {
        ...
        var settingsStore = new JsonSettingsRepository(context.ProjectRootPath, new JsonFileStore());
        var tagCatalogRepository = new JsonTagCatalogRepository(context.ProjectRootPath, new JsonFileStore());
        var item = CreateItem(
    ...
    public async Task ExportAsync_and_ImportAsync_round_trip_preserves_item_contract()
    {
        ...
        var sourceSettingsStore = new JsonSettingsRepository(source.ProjectRootPath, new JsonFileStore());
        var sourceTagCatalogRepository = new JsonTagCatalogRepository(source.ProjectRootPath, new JsonFileStore());
        var item = CreateItem(
            id: Guid.Parse("0f3a9d94-4df0-47f7-95c1-0f967c22f4db"),
            publicRef: "t-0426-1",
            collection: ItemCollection.Today);
        ...
        var importedSettingsStore = new JsonSettingsRepository(target.ProjectRootPath, new JsonFileStore());
        Assert.Equal("dark", await importedSettingsStore.GetAsync("theme"));

        var importedTagCatalogRepository = new JsonTagCatalogRepository(target.ProjectRootPath, new JsonFileStore());
    ...
    private static JsonDataTransferService CreateService(TestContext context)
    {
        return new JsonDataTransferService(
            ...
            new JsonFileStore(),
            new JsonIndexService(context.ProjectRootPath));
    }

    private static JsonItemRepository CreateRepository(TestContext context)
    {
        return new JsonItemRepository(
            new FixedClock(CreatedAt),
            new MonthlyJsonPathResolver(context.ProjectRootPath),
            new JsonFileStore(),
            new JsonIndexService(context.ProjectRootPath));
    }
